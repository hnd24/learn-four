...
export default function Banner({languageCourse}: {languageCourse: string}) {
	...
	const {getDetailCourse, loading} = useGetDetailCourse();
	useEffect(() => {
		const fetchCourses = async () => {
			const courses = await getDetailCourse(languageCourse);
			setCourseInfo(courses);
		};
		fetchCourses();
	}, [languageCourse, getDetailCourse]);
	return (
		<>
			{loading && !courseInfo ? (
				<SkeletonBanner />
					...
					<div className="flex flex-col justify-center w-full md:w-1/2 gap-6 lg:gap-10">
						<div className="flex flex-col gap-6 md:gap-3">
							...
							<Hint side="bottom" label={description || ""}>
								<div className=" text-md md:text-lg text-gray-100 md:line-clamp-3 ">
									{description}
								</div>
							</Hint>
						</div>
								...
								<div className="flex items-center w-fit px-2 py-1 bg-gray-300/35 border rounded-sm ">
									<div className="w-full flex gap-1 text-gray-100">
										<Avatar className="size-6">
											...
											<AvatarFallback className="bg-azureBlue !text-white text-sm">
												C4
											</AvatarFallback>
										</Avatar>
